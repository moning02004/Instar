...
from apps.post import Post, File
...
class CommentTestCase(TestCase):
    ...
    def test_comment_crud(self):
        ...
        # 추가
        response = self.client.post(reverse_lazy('post:comment:create', args=(self.post.id,)), data={
            'content': '이쁘네'
        })
        ...
        response = self.client.post(reverse_lazy('post:comment:delete', args=(self.post.id, comment.id)))
        self.assertEqual(response.status_code, 302)

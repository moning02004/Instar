...
from apps.post.models import Post, File
...
class PostTestCase(TestCase):
    ...
    def test_post_crud(self):
        ...
        response = self.client.post(reverse_lazy('post:create'), format='multipart', data={
            'content': '테스트 1',
            'images': self.file
        })
        ...
        # 열람
        response = self.client.get(reverse_lazy('post:detail', args=(2,)))
        self.assertEqual(response.status_code, 200)

        # 수정
        response = self.client.post(reverse_lazy('post:update', args=(2,)), data={
            'content': '새로운 테스트 1'
        })
        self.assertEqual(response.status_code, 302)
        ...
        # 삭제
        response = self.client.post(reverse_lazy('post:delete', args=(2,)))
        self.assertEqual(response.status_code, 302)
        post = Post.objects.filter(pk=2)
    ...
    def test_create_no_login(self):
        ...
        self.assertURLEqual(response.url[:response.url.index('?')], reverse_lazy('account:login'))

    # update_different_author
    def test_update_different_author(self):
        response = self.client.post(reverse_lazy('post:update', args=(self.post.id,)), data={
            'content': '새로운 테스트 1',
        })
        self.assertURLEqual(response.url[:response.url.index('?')], reverse_lazy('account:login'))

        self.client.login(username='user2@a.com', password='password2')
        response = self.client.get(reverse_lazy('post:update', args=(self.post.id,)), data={
            'content': '새로운 테스트 1',
        })
    ...
    def test_delete_different_author(self):
        response = self.client.post(reverse_lazy('post:delete', args=(self.post.id,)), data={
            'content': '새로운 테스트 1',
        })
        self.assertURLEqual(response.url[:response.url.index('?')], reverse_lazy('account:login'))

        self.client.login(username='user2@a.com', password='password2')
        response = self.client.post(reverse_lazy('post:delete', args=(self.post.id,)))
        self.assertEqual(response.status_code, 403)

    def test_heart(self):
        self.client.login(username='user1@a.com', password='password1')

        response = self.client.post(reverse_lazy('post:heart', args=(self.post.id,)))
        self.assertEqual(response.status_code, 404)

        ajax_header = {'HTTP_X_REQUESTED_WITH': 'XMLHttpRequest'}
        response = self.client.post(reverse_lazy('post:heart', args=(self.post.id,)), **ajax_header)
        self.assertEqual(response.status_code, 200)

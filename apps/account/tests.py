...
class UserTestCase(TestCase):
    ...
    def test_follow(self):
        ...
        response = self.client.get(reverse_lazy('account:profile', args=(self.user1.id,)))
        ...
        response = self.client.post(reverse_lazy('account:follow', args=(self.user1.id, self.user2.id)), **ajax_header)
        self.assertEqual(response.status_code, 200)
